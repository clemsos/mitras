...
def get_censored_tweets_from_elastic_search(_index,_start,_end):
  # init es client
  es = elasticsearch.Elasticsearch(['http://localhost:9200/'])

  # get column names
  indices=es.indices.get_mapping(index=_index,doc_type="tweet")
  ...
  print("Got %d censored tweets" % res['hits']['total'])
  return res['hits']

# get_censored_tweets_from_elastic_search("weiboscope","2012-02-05","2012-04-20")

def get_censored_words_list():

    # get censored
    censored_list_path="/home/clemsos/Dev/mitras/out/SensitiveSinaWeiboSearchTerms.csv"
...
# for w in get_censored_words_list(): print w

def get_censored_words_from_list(_words):
  
  # get censored
  censored_words=get_censored_words_list()
  censored_results=[h for h in _words if h in censored_words]
  # for word in censored_results :
  #     print word
  return censored_results

# print get_censored_words_from_list(["BBC中文网"])
...
class SearchCrawler():
    ...
    def __init__(self, word, fetcher):
        #super.__init__()

        logger.info('fetch search: %s' % word)
        self.url = 'http://weibo.cn/search/?keyword=%s' % word

        self.fetcher = fetcher
        self.storage = FileStorage(word)
        self.word = word

        self.word_not_exist = False
        html = self._fetch(self.url)

        if html is None:
            self.word_not_exist = True
            self.error = True
            logger.info("html is none")
        elif not self._check_word_exist(html):
            self.error = True
            self.word_not_exist = True
            logger.info("word not exist: %s" % word)
        else:
            self.error = False
    ...
    def _crawl(self, url, parser_cls):
        def start(url):
            html = self._fetch(url)
            parser = parser_cls(html, self.word, self.storage)
            return parser.parse()
    ...
    def _check_word_exist(self, html):
        # TODO : fix word return empty page error
        # If user not exist or forbiddened by weibo, directly return False
        if u'抱歉，您当前访问的用户状态异常，暂时无法访问。' in html:
            self.error = True
            self.word_not_exist = True
    ...
    @property
    def weibo_link(self):
        return 'http://weibo.cn/search/?smblog=搜微博&keyword=%s' % self.word
    ...
    def crawl(self):
        ...
        # logger.info("start to fetch %s's info" % self.user)
        # self.crawl_info()
        logger.info("start to fetch search for : %s" % self.word)
        self.crawl_weibo()
    ...
    def run(self):
        ...
        try:
            ...
            logger.info('error when crawl: %s' % self.word)
            logger.exception(e)
        finally:
            if hasattr(self.storage, 'close'):
                self.storage.close()
